...
export function getTaskCommentNotificationTemplate(
  authorName: string,
  taskTitle: string,
  ...
  return renderEmailLayout({
    ...
    accent: '#F58220', // SquadLog brand color
    contactEmail,
    contentHtml: `
      <div style="margin-bottom: 30px;">
        <p class="p" style="font-size: 16px; color: #4b5563;">Hello,</p>
        <p class="p" style="font-size: 16px; color: #4b5563;">
          <strong>${escapeHtml(authorName)}</strong> left a comment on the task 
          <span style="color: #F58220; font-weight: 600;">"${escapeHtml(taskTitle)}"</span> 
          in the project <strong style="color: #111827;">${escapeHtml(projectName)}</strong>.
        </p>
      </div>
